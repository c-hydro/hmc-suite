...
from apps.generic_toolkit.lib_default_args import logger_name, time_format_datasets
        ...
        if loc_key in list(obj_variable_lut.keys()):
            ...
            if env_key not in list(obj_variable_env.keys()):
                ...
                log_stream.warning(
                    ...
                    loc_key + '" set with value ' + str(loc_value) +
                    ' ... DONE. VARIABLE "' + str(env_key) + '" IS UPDATED WITH NEW VALUE')

        else:
            log_stream.warning(
                ' ===> Environment variable "' +
                loc_key + '" with value "' + str(loc_value) + '" ... SKIPPED. VARIABLE IS NOT IN THE EXPECTED LIST')
